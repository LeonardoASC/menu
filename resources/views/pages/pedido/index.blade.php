...
<body>
    <div x-data="setup()" :class="{ 'dark': isDark }">
        @if (auth()->check())
            <x-navbarheader />
            <div class="md:ml-64 pt-14 dark:bg-gray-700">
        @endif
                        ...
                        <div class="flex gap-x-4">
                            ...
                            <div class="min-w-0 flex-auto">
                                <div class="flex justify-between">
                                    <p class="text-sm font-semibold leading-6 text-gray-900">{{ $pedido->nome }} -
                                        {{ $pedido->quantidade }}x</p>
                                    <p class="text-sm font-semibold leading-6 text-gray-900">R$ {{ $pedido->preco }}</p>
                                </div>
                                <div class="flex justify-between">
                                    <p class="text-sm font-semibold leading-6 text-gray-900">
                                        {{ $pedido->cliente->nome }}</p>
                                    <p class="text-sm font-semibold leading-6 text-gray-900">Mesa:
                                        {{ $pedido->comanda->mesa_id }}</p>

                                </div>
                                @can('cadastrar produto')
                                    <p class="text-sm font-semibold leading-6 text-gray-900">CPF:
                                        {{ $pedido->cliente->cpf }}</p>
                                @endcan
                                <p class="text-sm font-semibold leading-6 text-gray-900"></p>
                                <p class="mt-1 truncate text-xs leading-5 text-gray-500">Observação:
                                    {{ $pedido->observacao }}</p>
                                <div class="flex items-center justify-between">
                                    <div class="text-sm leading-6 text-gray-900 mr-2">{{ $pedido->status }}

                                    </div>
                                    <p class="text-sm leading-6 text-gray-900 mr-2">numero do pedido:
                                        {{ $pedido->id }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        @can('cadastrar produto')
                            <div class="">
                                <form id="pedidoForm-{{ $pedido->id }}"
                                    action="{{ route('pedido.update', ['pedido' => $pedido]) }}" method="post">
                                    @csrf
                                    @method('PATCH')
                                    <div class="text-sm flex justify-center mt-2">
                                        <button
                                            class="toggleModalButton bg-green-400 px-4 py-2 rounded-md text-white font-semibold tracking-wide cursor-pointer"
